...
\begin{figure}[modelOverview]
        ...
        \caption{
 %       	Model Overview. Since the total path (first as a plasma particle, then as a photon) is longer for high energies particles, the high energy emission is stretched compared to low energy emission.
 			Here the red cone represents the propagation volume of low energy plasma, while the blue cone stands for high energy radiators. The green disk is the observer. $\theta_2$ and $\theta_1$ are opening angles of high and low energy jets, $\chi$ is an observer's off-axis angle. Every ``particle'' emitted from the central engine should go through two stages before being observer: traveling in plasma to the radiation point, and then traveling to the observer as photons, If $\chi < \theta_1$, then due to the relativistic beaming effect, the majority of observed photons is radiated near the point $R_1$, which lies on the line from the central engine to the observer. However, it might be different for the high energy radiation. If $\theta_2 < \chi$, then there is no high energy plasma near $R_1$, and ``particles'' will have to move near the jet axis first, and then turn at emission to the observer's direction. For each moment of emission, this path is longer, therefore the high energy emission will be stretched with respect to the low energies.
        }
        \label{fig:modelOverview}
\end{figure}

Finally, this assumption is able to explain the non-trivial stretching factor of the GRB 090926A. The detailed computations will be described in the following sections, however, you can see why the stretching is happening on the figure \ref{fig:modelOverview}.
