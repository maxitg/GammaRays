...
\begin{itemize}
	...
	\item{Location error (table 2). Location errors are used to improve the accuracy of filtering, that is to avoid losing statistics by filtering too much.}
	\item{$T_{05}$ -- $T_{95}$ interval of the LAT-detected emission (table 4). We need precise intervals for two things. First, to understand where is the fixed point of the stretching, $T_{05}$ is used as a good estimate for that. Second, the burst interval is used to download events data from the data server.}
\end{itemize}
...
\begin{itemize}
	...
	\item{{\bf Coordinate system}. We use J2000 here.}
	\item{{\bf Search radius (degrees)}. The value of $60$ is used here. Events will be filtered precisely later on by using the point spread functions (PSFs) of the LAT.}
	\item{
		{\bf Observation dates}. We extend the duration of the burst by $15\%$ relative to the table \ref{tab:bursts} values to both past and future to have a decent safety margin. So the time range we fill in here is:
		\begin{align*}
	...
	\item{{\bf Time system}. MET is used.}
	\item{{\bf Energy range (MeV)}. The whole energy range is required for our study. So we fill in $30,\,300000$.}
	\item{{\bf LAT data type}. Extended, because we need transient photons for better statistics.}
	\item{{\bf Spacecraft data}. Checked. We need it for both events filtering, and calculation of the PSFs and exposure maps.}
\end{itemize}

For background estimation purposes, we also download observational data during a day before the burst, specifically in the time range:
\begin{align*}
...
The first one is \texttt{gtselect}. It filters the Earth limb emission. We run the tool with the following parameters\footnote{\url{http://fermi.gsfc.nasa.gov/ssc/data/analysis/scitools/explore_latdata_burst.html}}:
\begin{lstlisting}
