...
\begin{figure}
	...
	\end{subfigure}
	\caption{
		High and low energy light curves produced by the geometrical model. Parameter values are the same as discussed in the section \ref{sec:parameters}. Redshift $z = 1$. Observer's off-axis angles are $\chi = 0$ for the left figure, and $\chi = 0.01$ for the right one.
	}
...
Finally, we can use the tests to fit parameters of the model:

\begin{itemize}
	\item{
		$v$, the velocity of the shell, $\gamma = \frac{1}{\sqrt{1-v^2}} \gg 1$.
		
		Observations suggest values for relativistic factors of plasma is a range from $\gamma = 100$ to $\gamma = 1000$.
		We take $\gamma = 300$ as an order of magnitude estimate.
	}
	\item{
		$\eta_0$, which defines the luminosity scale.
		
		From photon counts.
	}
	\item{
		$r_0$, the characteristic jet length; $r_0 \ll \frac{1}{H\left(0\right)}$, $H\left(t\right)$ is the Hubble parameter;
		
		From durations.
	}
	\item{
		$n$, which determines the sharpness of the jet end, $n > 3$;


	}
	\item{
		$\omega_0$, a characteristic radiation frequency;

		Non-physical
	}
	\item{
		$\theta_0$, the opening angle of the jet for radiation with frequency $\omega_0$, $\theta_0 \ll 1$;


	}
	\item{
		$k$, which determines how much the opening angle changes with frequency, $k < 0$;

	}
	\item{
		$\alpha$, the bare spectral index, $\alpha < -2k - 1$
