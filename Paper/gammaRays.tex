...
Produced files contain the probability densities as a function of the angular distance to the source. But we need an integral value, that is given the distance $d$, we want to find a fraction of photons coming from this source, which are observed to be closer than $d$ to the source. To find that we sum over the probability densities:
\begin{equation*}
...
\end{equation*}
Here \texttt{pdf[i]} is the observation probability density in the point separated with the source by the angle \texttt{angles[i]}. And $p\left(n\right)$ is the probability to observe the photon closer to the source than \texttt{angles[n]}. We find probabilities for intermediate angles by linear interpolation. Finally, we can compute the inverse value (that is distance as a function of probability) by binary search.
