...
\section{Introduction}
	...
	In this study we continue this series of results and explore the spectral variations between high energy bands $(\unit[100]{MeV}, \unit[1]{GeV})$ and $(\unit[1]{GeV}, \unit[300]{GeV})$ (we call them low and high energy bands throughout the paper).
	...
	Results obtained with this approach are discussed in the section \ref{sec:data}.
	Then we propose a simple geometrical model to explain these results in section \ref{sec:model}.
	Finally, our study is summarized in the section \ref{sec:discussion}.

\section{Fermi LAT Data}
	\label{sec:data}

	We take the GRB list from the Fermi-LAT catalog \cite{Ackermann:2013zfa}, and use the technique introduced in \cite{Rubtsov:2011qq} to estimate background radiation in both energy bands.
...
\section{Model}
	...
	\begin{enumerate}
		...
		\item{
			...
			\end{equation*}
			$\eta$ is a number of particles emitted per volume per solid angle per frequency.
			It is a function of the distance $r$ from the central engine, of the off-axis angle $\theta$, and of the radiation frequency $\omega$.
		}
	\end{enumerate}

	The burst is fully specified by the following set of parameters (we assume these parameters to be the same for all bursts, the values are obtained by fitting to make possible observed bursts from section \ref{sec:data}):
	\begin{itemize}
		...
		\item{$r_0 = \unit[3.36 \times 10^6]{sec^3}$, the characteristic jet length; $r_0 \ll \frac{1}{H\left(0\right)}$, $H\left(t\right)$ is the Hubble parameter;}
	...
	Numerical simulations based on these assumptions are able to reproduce both low (like of GRB 090902B) and high (like of GRB 090926A) stretching factors (see fig. \ref{fig:sampleLightCurves}).

	To test the model agreement to observations, we computed a few more things in its framework:
	\begin{itemize}
		\item{
		...
		\item{
			...
			You can see the PDF and CDF on fig. \ref{fig:kappaDistribution}.
			This distribution doesn't contradict with the values of stretching factors computed in section \ref{sec:data}.
		}
		\item{
		...
		\item{
			There is a correlation between the fractions of high energy photons (that is high energy photon count divided by total photon count), stretching factors and off-axis angles of observers (see fig. \ref{fig:correlations}). It allows one to predict off-axis angles.
		}
	\end{itemize}
...
\section{Discussion}
	...
	\begin{enumerate}
		\item{
			The time stretching of GRB light curves between different high energy bands is confirmed with statistical significance of $3.3\sigma$.
		}
		\item{
			This time stretching can be explained by geometry of the jet, without introducing any new spectral components.
		}
		\item{
			All GRBs are assumed the same in their rest frames.
			There is no need to vary internal burst parameters.
		}
	\end{enumerate}

	There, however, directions for improvement:
	\begin{enumerate}
		\item{
			The model prediction and observations on fig. \ref{fig:correlations} are, though close, not exactly agree.
		}
		\item{
			Shapes of light curves produced by model differ considerably from observed ones.
		}
		\item{
			Assumption of energy dependence on off-axis angle should be better justified.
		}
		\item{
			Much more results can be obtained from the model (like energy spectrum, and its variation with time).
		}
	\end{enumerate}
...
{\bf Fig.~1.}~GRB 090902B results. Low energy radiation is stretched ($\kappa < 1$) with significance of $2.2\sigma$.

{\bf Fig.~2.}~GRB 090926A results. High energy radiation is stretched ($\kappa > 1$) with significance of $3.3\sigma$.
...
{\bf Fig.~5.}~Stretching factors histogram and CDF produced by our model. Bar lines (black) show observed stretching factors. The sample contains $4096$ bursts.

{\bf Fig.~6.}~Correlations between high energy photon ratios, off-axis angles and stretching factors found in the sample produces by our model. The sample contains $4096$ bursts. Black crosses show observations. This correlation allows one to estimate off-axis angles of observed bursts.
