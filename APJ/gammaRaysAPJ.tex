...
\shorttitle{Time stretching of the GeV emission of GRBs}
\shortauthors{Piskunov and Rubtsov}
...
\title{Time stretching of the GeV emission of GRBs: \\
    Fermi-LAT data vs geometrical model}
...
\author{Maxim S. Piskunov\altaffilmark{1,2} and Grigory I. Rubtsov\altaffilmark{2}}
\affil{Faculty of Physics, Moscow State University, Moscow, Russia}
\affil{Institute for Nuclear Research of the RAS, Moscow, Russia}

\email{maxit@ms2.inr.ac.ru}
\email{grisha@ms2.inr.ac.ru}
...
\altaffiltext{1}{Faculty of Physics, Moscow State University, Moscow, Russia.}
\altaffiltext{2}{Institute for Nuclear Research of the RAS, Moscow, Russia.}
...
\begin{abstract}
        ...
        $\unit[1]{GeV}$. These include $4$ bursts: \objectname{GRB 080916C}, \objectname{GRB 090510},
        \objectname{GRB 090902B}, and \objectname{GRB 090926A}.  We use the
        Kolmogorov-Smirnov test to compare the light curves in the two
        bands, $\unit[100]{MeV} < E < \unit[1]{GeV}$ and
        $E > \unit[1]{GeV}$. For \objectname{GRB 080916C} and
        \objectname{GRB 090510} the light curves in the two bands are statistically
        compatible. However, for \objectname{GRB 090926A}, the higher-energy light
        curve is stretched compared to the lower-energy one with a
        statistical significance of $3.3 \sigma$ and, for \objectname{GRB 090902B},
...
\keywords{gamma-ray burst: general --- gamma-ray burst: individual(\objectname{GRB 080916C},
\objectname{GRB 090510}, \objectname{GRB 090902B}, \objectname{GRB 090926A})}
...
review. In particular, there are indications that the total energy emitted in gamma-rays during a
burst is similar for the different GRBs within an order
of magnitude~\citep{postnov1999grb,Bloom:2003wy}.
One should keep in mind, though, that the width of the distribution differs for multiple authors, and, in fact, is model dependent. More recent results, for example, suggest that energetics of different bursts might be different~\citep{racusin2009jet, chandra2012radio}. In this paper, however, we explore the possibility that most of the
...
Burst Catalog. Namely, \objectname{GRB 080916C} \citep{Tajima:2009az}, \objectname{GRB 090510}
\citep{Ackermann:2010us}, \objectname{GRB 090902B} \citep{Abdo:2009pg} and
\objectname{GRB 090926A} \citep{Bregeon:2011bu}. For these bursts we stretch the
...
(no stretching) for the \objectname{GRB 080916C} and \objectname{GRB 090510}. For the other two
bursts the stretching factor of 1 is, however, excluded.  For \objectname{GRB 090902B}
...
\objectname{GRB 090926A}. The high energy light curve of \objectname{GRB 090926A} is stretched
with respect to the low energy one by a factor of at least 1.99 (see
fig. \ref{fig:grb090926A}).  The stretching of the \objectname{GRB 090926A} high
...
authors {\bf \citep{Meszaros:1997je,Soderberg:2001mk,Nakamura:2001kd,Shen:2005ea,Shenoy:2013cba,granot2002off}}.  These
...
show that the stretching factors of \objectname{GRB 090902B} and \objectname{GRB 090926A} appear
...
Out of these 4 bursts, two (\objectname{GRB 080916C} and \objectname{GRB 090510}) have
stretching factors compatible with $\kappa = 1$ within $2\sigma$
range.

\objectname{GRB 090926A} has, however, high energy radiation stretched with respect
to low energy radiation (that is $\kappa > 1$). In contrast to that,
\objectname{GRB 090902B} has low energy radiation stretched with respect to high
...
\begin{itemize}
	...
	\item { The stretching factor of the burst should be
          compatible with the value for the \objectname{GRB 090926A}.  }
	...
	\item { We assume that all bursts have the same burst
          ...
          small stretching factors like \objectname{GRB 090902B} should be possible
          to observe. So we require that bursts with the stretching
          factor of \objectname{GRB 090902B} or lower appear in random samples by
...
\begin{enumerate}
	\item{
		Set $\gamma = 300$, $\omega_0 = \unit[1]{GeV}$, $z = 2.1062$ (which is the redshift of \objectname{GRB 090926A})
	}
	\item{
		Set $\eta_0$ and $r_0$ such that the duration of the burst, and the total number of observed photons are compatible with \objectname{GRB 090926A}, $T_{0.99}\left(z,\chi;\unit[0.1]{GeV},\infty\right) = \unit[219.5]{sec}$ and $p_{\infty} \left(z,\chi;\unit[0.1]{GeV},\infty\right) = 179.996$
	}
	\item{
...
stretching factors smaller than $1$ (like of \objectname{GRB 090902B}) and larger
than $1$ (like of \objectname{GRB 090926A}) depending on redshift and observer's
...
stretching factor of \objectname{GRB 090926A} is higher that the model
...
especially large stretching factor of \objectname{GRB 090926A}).  This means that
