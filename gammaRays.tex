...
The stretching factor for a continuous light curve is defined exactly like the stretching factor for a discrete one. It is the value $\kappa$ which makes the KS-distance minimal:
\begin{equation}
...
But the maximum of an absolute value cannot be differentiated, so it is a complicated thing to compute numerically. Lets instead rewrite the expression for $\kappa$ in terms of a positive and a negative KS-distances:
\begin{align*}
...
\end{equation}
And this value of $\kappa$ also makes the $\max\left({D_+, -D_-}\right)$ minimal, since $D_+$ monotonously increase, and $-D_-$ monotonously decrease with $\kappa$.

So we have now a simplified way to compute $\kappa$ by solving an equation instead of computing the minimum. And $\Phi$ can be differentiated, which makes it easier to compute maximums and minimums of their differences.

Being able to compute the stretching factor, we could now compare our model predictions with observations given the position of an observer. However, we don't know the observer's off-axis angle $\chi$, so we cannot check the stretching factor prediction directly. Instead, we will focus on a series of tests, which will ensure that our model doesn't contradict existing observations. These tests will be discussed in the following sections.
