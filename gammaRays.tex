...
However, the largest contribution to the total energy in our model is due to unobservable low-energy photons. Moreover, the total energy integral diverges on low energies as we will see later. This divergence is understandable, since our model is only valid only for large energy photons. Still, we have to ensure that the total energy of radiated high energy photons is low enough.

To do that, we have to modify the expression for the radiation intensity, to cut off all the radiators with frequencies outside the $\left(\omega_1, \omega_2\right)$ interval:
\begin{equation}
	\eta_{\omega_1}^{\omega_2}\left(r,\theta,\omega\right) = \begin{cases}
		0 & \text{if } \omega < \omega_1 \\
		\eta\left(r,\theta,\omega\right) & \text{if } \omega_1 < \omega < \omega_2 \\
		0 & \text{if } \omega_2 < \omega
	\end{cases}
\end{equation}
