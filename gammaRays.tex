...
For our set of parameters, this algorithm produced a sample, empirical CDF of which you can see on fig. However, the error margins of the observed stretching factors are too large to use the KS-test on them. Nevertheless, It appears that our model doesn't contradict existing observations.

\subsection{Very high energy bursts fraction}

Our final test is to calculate how much of the observable bursts can be observed in the very high energy range -- GeV and above.
...
\end{align}
Here $z_\text{max}\left(\omega_1,\omega_2\right)$ and $\chi_\text{max}\left(z,\omega_1,\omega_2\right)$ are the same values we used in the previous sections: the maximum redshift from which a burst can be observed, and the maximum off-axis angle with which the burst at redshift $z$ can be observed.
