...
The second test is to calculate the stretching factors distribution of the observable bursts, and to compare it with observations.
...
Instead, we will use the Monte-Carlo method to produce a large representative sample of stretching factors. Then, we will calculate an empirical CDF of this sample, and compare it with observations using the KS-test. Since we can compute much larger sample than that of observations, we will not lose much precision due to this simplification.

We still have one ingredient missing though -- the evolution of the bursts density. We assume that the bursts density is roughly proportional to the stars density, and the stars density is roughly proportional to the matter density, which changes as $\left(1+z\right)^3$. It is clear, however, that since no stars existed at very small redshifts, the burst density should decline there, so we add an exponential cutoff to it:
\begin{equation}
...
Now when we have all the ingredients, we can describe the algorithm for computing a sample of stretching factors:
\begin{enumerate}
\item{
	Select a redshift $z$. The distribution of redshifts is determined by the bursts count in a given volume. Its CDF is:
	\begin{equation}
	...
	\end{equation}
	Here $\left(z_\text{min}, z_\text{max}\right)$ is the range from which we are selecting redshifts. $z_\text{min}$ should be some small number so it is unlikely to detect a burst with $z < z_\text{min}$. And $z_\text{max}$ is the largest redshift from which a burst can be observed:
	\begin{equation}
	p_\infty\left(z_\text{max}, 0., \omega_2, \omega_3\right) = p_\text{min}
	\end{equation}
	Here $p_\text{min}$ is the smallest number of particles required to claim an observation.

	Now we can select a properly distributed random redshift by solving an equation for $z$:
	\begin{equation}
	\Phi_z\left(z\right) = x
	\end{equation}
	where $x$ is a uniformly distributed random number in the range $0$ to $1$.
}
\item{
	Select an off-axis angle $\chi$. All directions for the burst jets are equally probable. So the CDF of the off-axis angles distribution is:
	\begin{equation}
	...
	\end{equation}
	Here $\chi_\text{max}$ is the largest off-axis angle with which a burst at redshift $z_\text{min}$ can be observed:
	\begin{equation}
	p_\infty\left(z_\text{min}, \chi_\text{max}, \omega_2, \omega_3\right) = p_\text{min}
	\end{equation}

	Now as with $z$ distribution we can find the random $\chi$ by solving the equation:
	\begin{align*}
	\Phi_\chi\left(\chi\right) = \left(\frac{\chi}{\chi_\text{max}}\right)^2 = y \\
	\chi = \chi_\text{max}\sqrt{y}
	\end{align*}
	where $y$ is a uniformly distributed random number in the range $0$ to $1$.

	Note that $p\left(z,\chi\right)$ decreases with $z$, as well as with $\chi$, so all observable burst configurations can be generated except for the unlikely $z<z_\text{min}$ case.
}
\item{If the burst is observable: $p_\infty\left(z,\chi,\omega_2,\omega_3\right) > p_\text{min}$, add $\kappa\left(z,\chi\right)$ to the sample.}
\item{Repeat from the step $1$.}
\end{enumerate}

For our set of parameters, this algorithm produced a sample, empirical CDF of which you can see on fig. The error margins of the observed stretching factors are too large to use the KS-test on them. It appears, however, that our model doesn't contradict existing observations.
